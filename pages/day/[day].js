...
const DAYS = {
  // ... (Your existing DAYS object content remains unchanged)
  '1': {
...
export default function Day(){
  ...
  // HYDRATION FIX: Prevent rendering until router.query is available
  if (!day) {
  ...
  async function handleSubmit(isAuto=false){
    ...
    // Use SUBMISSION_FORM_ACTION (the /formResponse endpoint) for the submission URL
    const action = CONFIG.SUBMISSION_FORM_ACTION
    const fields = CONFIG.SUBMISSION_FIELDS || {}
    const dayCode = info.code

    // Ensure action URL and fields exist
    if(action && Object.keys(fields).length>0){
      const params = []
      
      // Build the URL parameters with the entry IDs and values
      ...
      // 💡 CHANGE: We are now *only* using the window.open method, targeting the /formResponse
      //    endpoint with the data appended. This is the most reliable way to force a submission
      //    when direct fetch POST is blocked by CORS.
      window.open(url, '_blank')
    }
    
    // Final user alert (occurs immediately, even if submission is in new tab)
  ...
  return (
    <div>
      <div className="card">
        ...
        {/* PARTICIPANT DETAILS (Editable before Question is shown) */}
        ...
        {/* QUESTION & ANSWER SECTION (Hidden until 'Show Question' is clicked) */}
        {showQuestion ? (
          <>
            <p className="small">
